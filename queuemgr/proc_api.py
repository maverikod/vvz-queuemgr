...
from .proc_manager import ProcManager, ProcManagerConfig, ProcManagerContext
...
class ProcQueueSystem:
    ...
    def __init__(self, registry_path: str = "queuemgr_registry.jsonl", 
    ...
    def start(self) -> None:
        ...
        if self._is_initialized:
            raise ProcessControlError("queue", "start", Exception("Queue system is already running"))
    ...
    def is_running(self) -> bool:
        ...
        return self._is_initialized and self._manager is not None and self._manager.is_running()
        
    def add_job(self, job_class: Type[QueueJobBase], job_id: str, 
    ...
    def _ensure_running(self) -> None:
        ...
        if not self.is_running():
            raise ProcessControlError("queue", "operation", Exception("Queue system is not running"))
...
def get_proc_queue_system(registry_path: str = "queuemgr_registry.jsonl",
                         proc_dir: str = "/tmp/queuemgr",
                         shutdown_timeout: float = 30.0) -> ProcQueueSystem:
    """
...
@contextmanager
def proc_queue_system(registry_path: str = "queuemgr_registry.jsonl",
                     proc_dir: str = "/tmp/queuemgr",
                     shutdown_timeout: float = 30.0):
    """
...
def start_proc_queue_system(registry_path: str = "queuemgr_registry.jsonl",
                           proc_dir: str = "/tmp/queuemgr",
