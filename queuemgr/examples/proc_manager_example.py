...
import time
from queuemgr.proc_api import proc_queue_system, add_job, start_job, get_job_status, list_jobs
...
class LinuxJob(QueueJobBase):
    ...
    def execute(self) -> None:
        ...
        print(f"Linux Job {self.job_id} starting work for {self.work_duration} seconds...")
...
def main():
    ...
    with proc_queue_system(
        registry_path="linux_registry.jsonl",
        proc_dir="/tmp/queuemgr_linux"
    ) as queue:
        # Add some jobs
        queue.add_job(LinuxJob, "linux-job-1", {"duration": 3, "process_name": "Worker-1"})
        queue.add_job(LinuxJob, "linux-job-2", {"duration": 5, "process_name": "Worker-2"})
        queue.add_job(LinuxJob, "linux-job-3", {"duration": 2, "process_name": "Worker-3"})
    ...
    start_proc_queue_system(
        registry_path="linux_registry2.jsonl",
        proc_dir="/tmp/queuemgr_linux2"
    )
    
    try:
        # Add and start jobs using global functions
        add_job(LinuxJob, "global-linux-job-1", {"duration": 2, "process_name": "Global-1"})
        add_job(LinuxJob, "global-linux-job-2", {"duration": 3, "process_name": "Global-2"})
