...
class IssueDetector:
    ...
    def check_method_issues(self, node: ast.FunctionDef, file_path: str, class_name: str = None) -> None:
    ...
    def _check_any_type_usage(self, node: ast.FunctionDef, file_path: str, class_name: str = None) -> None:
        ...
        if node.returns:
            if isinstance(node.returns, ast.Name) and node.returns.id == "Any":
                self.issues["any_type_usage"].append({
                    "file": file_path, "class": class_name, "method": node.name,
                    "type": "return_type", "line": node.lineno, "description": "Return type is Any"
                })
        ...
        for arg in node.args.args:
            if arg.annotation:
                if isinstance(arg.annotation, ast.Name) and arg.annotation.id == "Any":
                    self.issues["any_type_usage"].append({
                        "file": file_path, "class": class_name, "method": node.name,
                        "type": "parameter", "parameter": arg.arg, "line": node.lineno,
    ...
    def _check_generic_exception_usage(self, node: ast.FunctionDef, file_path: str, class_name: str = None) -> None:
    ...
    def _check_statement_for_generic_exception(self, stmt: ast.AST, file_path: str, class_name: str, method_name: str) -> None:
        ...
        if isinstance(stmt, ast.Try):
                ...
                if handler.type:
                    if isinstance(handler.type, ast.Name) and handler.type.id == "Exception":
                        self.issues["generic_exception_usage"].append({
                            ...
                            "line": handler.lineno, "type": "except_clause",
                    ...
                    self.issues["generic_exception_usage"].append({
                        "file": file_path, "class": class_name, "method": method_name,
                        "line": handler.lineno, "type": "bare_except",
            ...
            for stmt_in_try in stmt.body:
                self._check_statement_for_generic_exception(stmt_in_try, file_path, class_name, method_name)
            ...
            if stmt.exc:
                if isinstance(stmt.exc, ast.Name) and stmt.exc.id == "Exception":
                    self.issues["generic_exception_usage"].append({
                        "file": file_path, "class": class_name, "method": method_name,
                        "line": stmt.lineno, "type": "raise_exception",
        ...
        if hasattr(stmt, 'body') and isinstance(stmt.body, list):
            for nested_stmt in stmt.body:
                self._check_statement_for_generic_exception(nested_stmt, file_path, class_name, method_name)
